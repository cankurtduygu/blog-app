...
export default function CardDetail() {
  const { id } = useParams();
  const { getBlogsById, toggleLike } = useBlogCall();
    ...
    <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-6 md:p-10 mt-10">
      ...
      <div className="mt-8">
        ...
        {comments?.length ? (
            ...
            {comments.map((c) => (
              <li key={c._id} className="bg-gray-100 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-brandDark font-bold">{c.username}</span>
          ...
          <div className="text-brandDark/60 text-center">No comments yet.</div>
        )}
      </div>
    </div>
  );
